...
class Reloj(QWidget):
    def __init__(self):
        ...
        self.hora = 3  # Inicializa la hora en 3
        self.minuto = 0  # Inicializa los minutos en 0
        self.initUI()  # Llama al método para inicializar la interfaz gráfica

    # Método para configurar la interfaz gráfica
    def initUI(self):
        self.setWindowTitle('Wind Chart EpiC')  # Establece el título de la ventana
        self.setGeometry(100, 100, 400, 400)  # Define la geometría de la ventana (posición x, y, ancho, alto)
        ...
        # Añadir un espaciador para la circunferencia (será manejado en paintEvent)
        layout.addStretch()

        layout.addWidget(self.resultado)  # Añade el campo de texto al layout 
        # Establecer márgenes (izquierda, arriba, derecha, abajo)
        layout.setContentsMargins(30, 9, 30, 9)  # Margen de 20px a los lados y abajo                       -
    ...
    def paintEvent(self, event):
        ...
        self.drawReloj(painter, rect)  # Llama al método para dibujar el reloj
        self.drawManecillas(painter, rect)  # Llama al método para dibujar las manecillas


    # Método para dibujar el fondo del reloj
    def drawReloj(self, painter, rect):
            ...
            outer_radius = min(rect.width(), rect.height() - 65) // 2 - 10  # Calcula el radio del círculo exterior (1/5 del tamaño mínimo del rectángulo)
            ...
            painter.setPen(QPen(QColor(255, 215, 0)))  # Cambia el color a dorado y el grosor a 2 para las líneas de división
    ...
    def drawManecillas(self, painter, rect):  # Método para dibujar las manecillas del reloj
        painter.setRenderHint(QPainter.Antialiasing)  # Habilita el suavizado de bordes

        center = rect.center()  # Obtiene el centro del área de dibujo
        radius = min(rect.width(), rect.height()) // 3  # Calcula el radio del reloj

        # Dibujar manecilla de la hora
        hour_angle = (self.hora % 12 + self.minuto / 60) * 30  # Calcula el ángulo de la manecilla de la hora
        hour_length = radius * 0.6  # Longitud de la manecilla de la hora
    ...
    def mousePressEvent(self, event: QMouseEvent):  # Método que se llama al hacer clic en la ventana
        ...
        angle = (math.degrees(math.atan2(-click_y, click_x)) + 360) % 360  # Convierte la posición en un ángulo
        ...
        self.hora = (self.hora + 3) % 12  # 0 grados corresponde a las 3 en punto
        self.hora = 12 if self.hora == 0 else self.hora  # Para mostrar 12 en lugar de 0

        self.resultado.setText(f"+ {self.hora}:{self.minuto:02d}")  # Actualiza el campo de texto con la hora seleccionada       -
        self.update()  # Solicita repintar la ventana

if __name__ == '__main__':  # Comprueba si este archivo es el principal
    app = QApplication(sys.argv)  # Crea una instancia de QApplication
    reloj = Reloj()  # Crea una instancia de la clase Reloj
    reloj.show()  # Muestra el reloj
